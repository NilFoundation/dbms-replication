...
auto replicated_state::to_string(leader_internal_state state) noexcept -> std::string_view {
    switch (state) {
...
auto replicated_state::to_string(follower_internal_state state) noexcept -> std::string_view {
    switch (state) {
...
void state_status::toVelocyPack(velocypack::Builder &builder) const {
    std::visit([&](auto const &s) { velocypack::serialize(builder, s); }, variant);
}

auto state_status::fromVelocyPack(velocypack::Slice slice) -> state_status {
    auto role = slice.get(static_strings::StringRole).stringView();
    if (role == StaticStrings::Leader) {
        return state_status {velocypack::deserialize<leader_status>(slice)};
    } else if (role == StaticStrings::Follower) {
        return state_status {velocypack::deserialize<follower_status>(slice)};
    } else if (role == static_strings::StringUnconfigured) {
        return state_status {velocypack::deserialize<unconfigured_status>(slice)};
    } else {
...
auto follower_internal_state_string_transformer::toSerialized(follower_internal_state source, std::string &target) const
    -> inspection::Status {
    target = to_string(source);
    return {};
}

auto follower_internal_state_string_transformer::fromSerialized(std::string const &source,
...
auto leader_internal_state_string_transformer::toSerialized(leader_internal_state source, std::string &target) const
    -> inspection::Status {
    target = to_string(source);
    return {};
}

auto leader_internal_state_string_transformer::fromSerialized(std::string const &source,
                                                              leader_internal_state &target) const
    -> inspection::Status {
    if (source == StringUninitializedState) {
