...
#include "leader_state_manager.tpp"
#include "follower_state_manager.tpp"
#include "unconfigured_state_manager.tpp"
#include "state_interfaces.hpp"
...
namespace nil::dbms::replication::replicated_state {
    ...
    } catch (replication::replicated_log::participant_resigned_exception const &ex) {
    ...
    auto replicated_state_t<S>::guarded_data::runLeader(std::shared_ptr<replicated_log::ilog_leader> logLeader,
        ...
        auto manager = std::make_shared<leader_state_manager<S>>(std::move(loggerCtx), _self.shared_from_this(),
                                                                 std::move(logLeader), std::move(core),
                                                                 std::move(token), _self.factory);
        currentManager = manager;
