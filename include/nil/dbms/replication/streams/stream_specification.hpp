...
namespace nil::dbms::replication::streams {
    ...
    struct stream_descriptor<StreamId, Type, tag_descriptor_set<TDs...>> {
        ...
        static_assert((
            std::is_invocable_r_v<Type, typename TDs::deserializer, serializer_tag_t<Type>, velocypack::Slice> && ...));
        ...
        static_assert((std::is_invocable_r_v<void,
                                             typename TDs::serializer,
                                             serializer_tag_t<Type>,
    ...
    template<typename... Descriptors>
    struct is_stream_descriptor_set<stream_descriptor_set<Descriptors...>> : std::true_type { };
    template<typename S>
    ...
    template<typename T>
    using stream_descriptor_primary_tag_t = typename tag_descriptor_set_primary_t<stream_descriptor_tags_t<T>>::type;
    ...
    namespace detail {
        ...
        struct stream_descriptor_by_id_impl<StreamId, D, Ds...>
            : std::conditional<StreamId == stream_descriptor_id_v<D>,
                               D,
    ...
    template<StreamId StreamId, typename Ds>
    using stream_type_by_id_t = stream_descriptor_type_t<stream_descriptor_by_id_t<StreamId, Ds>>;
    ...
    namespace detail {
        ...
        struct stream_index_by_id_impl<I, StreamId, D> : std::integral_constant<std::size_t, I> { };
    }    // namespace detail
    ...
    struct stream_index_by_id<StreamId, stream_descriptor_set<Ds...>> {
        static_assert(((stream_descriptor_id_v<Ds> == StreamId) || ...));
        static inline constexpr std::size_t value = detail::stream_index_by_id_impl<0, StreamId, Ds...>::value;
    };
