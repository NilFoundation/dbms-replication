...
namespace nil::dbms::replication::streams {
    ...
    template<StreamId Id, typename Type, typename Tags>
    auto stream_information_block<stream_descriptor<Id, Type, Tags>>::get_iterator_range(log_index start,
                                                                                         log_index stop)
        ...
        struct Iterator : typed_log_range_iterator<StreamEntryView<Type>> {
            ...
            explicit Iterator(ContainerType log, log_index start, log_index stop) :
                _log(std::move(log)), _current(std::lower_bound(std::begin(_log), std::end(_log), start,
                                                                [](StreamEntry<Type> const &left, log_index index) {
