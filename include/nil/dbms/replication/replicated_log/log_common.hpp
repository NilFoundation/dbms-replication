...
namespace nil::dbms::replication {
    ...
    namespace replicated_log {
        ...
        struct commit_fail_reason {
            ...
            static auto with_quorum_size_not_reached(quorum_size_not_reached::who_type who,
            ...
            static auto withFewerParticipantsThanWriteConcern(fewer_participants_than_write_concern)
                -> commit_fail_reason;
        ...
        template<class Inspector>
        auto inspect(Inspector &f, nil::dbms::replication::replicated_log::commit_fail_reason &x) {
            if constexpr (Inspector::isLoading) {
        ...
        auto operator<<(std::ostream &, commit_fail_reason::quorum_size_not_reached::participant_info)
            -> std::ostream &;
...
namespace nil::dbms {
    ...
    template<>
    struct velocypack::Extractor<replication::LogId> {
        static auto extract(velocypack::Slice slice) -> replication::LogId {
            return replication::LogId {slice.getNumericValue<std::uint64_t>()};
        }
    };
...
template<>
struct fmt::formatter<nil::dbms::replication::LogId> : fmt::formatter<nil::dbms::basics::Identifier> { };

template<>
struct std::hash<nil::dbms::replication::log_index> {
    [[nodiscard]] auto operator()(nil::dbms::replication::log_index const &v) const noexcept -> std::size_t {
...
struct std::hash<nil::dbms::replication::log_term> {
    [[nodiscard]] auto operator()(nil::dbms::replication::log_term const &v) const noexcept -> std::size_t {
        return std::hash<uint64_t> {}(v.value);
    }
};

template<>
struct std::hash<nil::dbms::replication::LogId> {
    [[nodiscard]] auto operator()(nil::dbms::replication::LogId const &v) const noexcept -> std::size_t {
