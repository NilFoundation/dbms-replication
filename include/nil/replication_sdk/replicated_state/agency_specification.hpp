...
namespace nil::dbms::replication_sdk::replicated_state::agency {
    ...
    struct implementation_spec {
        ...
        bool operator==(const implementation_spec &rhs) const {
    ...
    auto inspect(Inspector &f, implementation_spec &x) {
    ...
    struct Current {
        struct participant_status {
            state_generation generation;
            snapshot_info snapshot;    // TODO might become an array later?
    ...
    struct Target {
        ...
        std::unordered_map<ParticipantId, Participant> participants;
        nil::dbms::replication_sdk::agency::log_target_config config;
        std::optional<std::uint64_t> version;
