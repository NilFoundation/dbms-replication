...
namespace nil::dbms::replication_sdk {
    ...
    struct replicated_log_methods {
        ...
        using GenericLogStatus =
            std::variant<replication_sdk::replicated_log::log_status, replication_sdk::replicated_log::global_status>;
        ...
        virtual auto get_replicated_logs() const
            -> futures::Future<std::unordered_map<nil::dbms::replication_sdk::LogId,
                                                  std::variant<replicated_log::log_status, ParticipantsList>>> = 0;
        virtual auto getLocalStatus(LogId) const -> futures::Future<replication_sdk::replicated_log::log_status> = 0;
        virtual auto getGlobalStatus(LogId, replicated_log::global_status::SpecificationSource) const
            -> futures::Future<replication_sdk::replicated_log::global_status> = 0;
        ...
        virtual auto poll(LogId, log_index, std::size_t limit) const
            -> futures::Future<std::unique_ptr<persisted_log_iterator>> = 0;
        virtual auto head(LogId, std::size_t limit) const -> futures::Future<std::unique_ptr<persisted_log_iterator>> = 0;
        virtual auto tail(LogId, std::size_t limit) const -> futures::Future<std::unique_ptr<persisted_log_iterator>> = 0;
