...
namespace nil::dbms::replication {
    ...
    namespace log {
        ...
        struct CommitFailReason {
            ...
            static auto withQuorumSizeNotReached(QuorumSizeNotReached::who_type who, term_index_pair spearhead) noexcept
                -> CommitFailReason;
            static auto withForcedParticipantNotInQuorum(ParticipantId who) noexcept -> CommitFailReason;
            static auto
                withNonEligibleServerRequiredForQuorum(NonEligibleServerRequiredForQuorum::CandidateMap) noexcept
                -> CommitFailReason;
            // This would have too many `std::size_t` arguments to not be confusing,
            // so taking the full object instead.
            static auto withFewerParticipantsThanWriteConcern(FewerParticipantsThanWriteConcern) -> CommitFailReason;
        ...
        auto operator<<(std::ostream &, CommitFailReason::QuorumSizeNotReached::ParticipantInfo) -> std::ostream &;
...
struct std::hash<nil::dbms::replication::log_id> {
    [[nodiscard]] auto operator()(nil::dbms::replication::log_id const &v) const noexcept -> std::size_t {
